...
# Returns SE object
# (Demand lots of resources. Can take tens of minutes, for me it took over hour.) 
# Might be best to save it in data, so this step can be skipped? ################
#se_dmn <- runDMN(tse, name = "DMN", k = 1:7) 
data(se_dmn)
...
getBestDMNFit(se_dmn, type = "laplace")
```

```{r}
# Plots the DMN
plot_dmn_fit <- plotDMNFit(se_dmn, type = "laplace")
...
```{r}
# Does the grouping. Returns DMNGroup object. Genotype is used for 
# grouping. 
#dmn_group <- calculateDMNgroup(se_dmn, variable = "Genotype", 
#                             exprs_values = "counts", k = 2)

# Does not do anything, can be deleted

```

```{r}
# Weights
DirichletMultinomial::mixturewt(getBestDMNFit(se_dmn))
```

```{r}
# Samples' probabilities that tells the likelihood to belong to component
head(DirichletMultinomial::mixture(getBestDMNFit(se_dmn)))
```

```{r}
# Contribution of samples to each component
head(DirichletMultinomial::fitted(getBestDMNFit(se_dmn)))
```
...
# Creates a vector that contains information whether it is more probable that
# sample belongs to component 1 or 2.
vec <- rep(0, length(prob))
vec[prob[,1] > prob[,2]] <- "comp1"
vec[prob[,2] > prob[,1]] <- "comp2"

# Creates a data frame that contains principal coordinates and DMM information
bray_curtis_dmm_pcoa_df <- cbind(bray_curtis_pcoa_df,
                             dmm_component = vec)

# Creates a plot
bray_curtis_dmm_plot <- ggplot(data = bray_curtis_dmm_pcoa_df, 
                                   aes(x=pcoa1, y=pcoa2,
                                       color = dmm_component)) +
  geom_point() +
  xlab("Coordinate 1") + # x axis title
  ylab("Coordinate 2") + # y axis title
  ggtitle("Bray-Curtis PCoA with DMM clustering") # main title

bray_curtis_dmm_plot
