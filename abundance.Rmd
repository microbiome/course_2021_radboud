...
Wilcoxon test tests if there is difference between two groups. It is equivalent to 
t-test. However, t-test is parametric and Wilcoxon test is non-parametric, which means
that Wilcoxon test does not require normally distributed data. It is always wise to 
check if the data is normally distributed. If it is not then the data should be normalized. 
If that is not possible then non-parametric equivalents, like Wilcoxon test, should be used.
...
```{r}
# Does Wilcoxon test. Does it only for columns that contain abundances, not for
...
for (column in 
  ...
  result <- wilcox.test(abundance_analysis_data[,column] ~ patient_status, data = abundance_analysis_data)
  
  # Stores p-value to a vector
  wilcoxon_p  <- c(wilcoxon_p , result$p.value)
}

wilcoxon_p <- data.frame(taxa = 
...
```{r}
# Does the adjustment
wilcoxon_p$p_holm <- p.adjust(wilcoxon_p$p_raw, method = "holm")
```

```{r}

df = data.frame(x = c(wilcoxon_p$p_raw, wilcoxon_p$p_holm), 
                type=rep(c("raw", "holm"), c(length(wilcoxon_p$p_raw), length(wilcoxon_p$p_holm))))

wilcoxon_plot <- ggplot(df) + geom_histogram(aes(x=x, fill=type)) +
  xlab("p-value") + 
  ylab("Frequency")
...
### DESeq2
Our second analysis method is DESeq2. It performs the data normalization  automatically. 
It also takes care of the p-value adjustment, so we don't have to worry about that.
...
```{r}
mf <- data.frame(df$padj, wilcoxon_p$p_holm)
p <- ggplot(mf, aes(x = df$padj, y = wilcoxon_p$p_holm)) +
...
print(paste0("Wilcoxon test p-values under 0.05: ", sum(wilcoxon_p$p_holm<0.05, na.rm = TRUE), "/", length(wilcoxon_p$p_holm)))
```
