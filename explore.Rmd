...
the beginning of the table. rowData seems to contain information from 6 different 
taxonomy classes.
...
colData slot contains sample metadata. It contains information for all 27 samples.
...
Abundances are always relative even though counts are calculated. That is because 
abundances always depends, for example, on settings of instruments, and also
how well sample was drawn. That is why relative abundances are usually more informative
than plain counts.
...
firmicutes_abund_plot <- ggplot(firmicutes_abund_df, aes(x = abund)) + 
  ...
  xlab("Relative abundance") + # x axis title
