...
# about patients' status. There two groups "ADHD" and "control". 
# Let's include that to the data frame.
abundance_analysis_data <- cbind(abundance_analysis_data, 
...
```{r}
colnames <- names(abundance_analysis_data[, !names(abundance_analysis_data) %in% 
                                            "patient_status"])
...
# Orders the rows of data frame in increasing order firstly based on 
# column "log2FoldChange" and secondly based on "padj" column
...
p <- ggplot(mf, aes(x = df$padj, y = wilcoxon_p$p_adjusted)) +
       labs(x = "DESeq2 adjusted p-value", 
            y = "Wilcoxon test adjusted p-value") +
       geom_count() +
...
```{r}
print(paste0("DESeq2 p-values under 0.05: ", 
             sum(df$padj<0.05, na.rm = TRUE), "/", length(df$padj)))
print(paste0("Wilcoxon test p-values under 0.05: ", 
...
# Some taxa names are that long that they don't fit nicely into title. 
# So let's add there a line break after e.g. "Genus". 
# Here the dot after e.g. Genus is replaced with ": \n"
...
```{r}
# Sorts p-values in increasing order. Takes 3rd first ones. 
# Takes those rows that match with p-values. Takes taxa. 
